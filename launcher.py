# coding:utf-8
"""
打包入口（PyInstaller 用这个）。

为什么不直接把 xk_app/main.py 当入口：PyInstaller 会把入口脚本当作顶层
__main__ 执行，脚本里的相对导入（from .selftest import ...）会报
"attempted relative import with no known parent package"。
所以这里只做一个薄薄的启动器，真正的东西都在 xk_app 包内。

交互上是这样的：
    TsinghuaBookCrawler.exe                      -> 图形界面
    TsinghuaBookCrawler.exe --selftest           -> 打包后自检
    TsinghuaBookCrawler.exe --screenshot <目录>   -> 渲染一张界面截图后退出（验收用）
    TsinghuaBookCrawler.exe <url> --token xxx    -> 命令行版本
"""
import os
import sys

# 冻结后 sys.path[0] 是 _MEIPASS，开发态是仓库根；两种情况下都能 import xk_app
_HERE = os.path.dirname(os.path.abspath(__file__))
if _HERE not in sys.path:
    sys.path.insert(0, _HERE)


def _screenshot(out_dir):
    """
    在真实显示上把界面画出来存成 PNG，然后退出。
    用来验收「打包之后界面确实是对的」，不依赖人工点开看。

    注意：这里会把配置目录顶到临时目录。窗口关闭时会把界面上的设置落盘，
    而这个函数为了截图会往设置里填一个假的 token —— 不隔离的话，
    ...
    from xk_app.app.gui import login as login_mod
...
def main_entry():
    ...
    if args and args[0] == '--screenshot':
        return _screenshot(args[1] if len(args) > 1 else os.getcwd())
    from xk_app.main import main
    return main()
